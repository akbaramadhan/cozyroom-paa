            ...
            <div class="row justify-content-center">
                <div class="col-md-8">
                    ...
                    <div class="search-box">
                        <button class="btn btn-primary me-2" onclick="location.href='{{ route('beranda') }}'">Back</button>
                    </div>
                    <div class="search-box">
                    ...
                    <div class="search-results" id="search-results">
                        @foreach ($data as $item)
                            <div class="card">
                                <a href="{{ route('kos.show_login', $item['id']) }}" class="card-link">
                                    <img src="{{ asset('storage/kos/' . $item['gambar']) }}" alt="{{ $item['kos'] }}"
                                        class="card-img-top">
                                    <div class="card-body">
                                        <h5 class="card-title">{{ $item['kos'] }}</h5>
                                        <p class="card-text">{{ $item['deskripsi'] }}</p>
                                        <p class="card-text"><small class="text-muted">{{ $item['lokasi'] }}</small></p>
                                        <p class="card-text"><strong>Rp. {{ $item['harga'] }}</strong></p>
                                    </div>
                                </a>
                                <a href="{{ route('produk-kos-update', $item['id']) }}"
                                    class="btn btn-warning btn-sm">Edit</a>
                                <button onclick="deleteKos({{ $item['id'] }})"
